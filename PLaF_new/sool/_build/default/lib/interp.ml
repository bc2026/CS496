...
let rec is_subclass (id1 : string) (id2 : string) : bool =
  if id1 = id2 then true
  else match List.assoc_opt id1 !g_class_env with
    | Some (superclass, _, _) -> is_subclass superclass id2
    | None -> false
  

let name_mangle n es = n ^ "_" ^ string_of_int (List.length es)
(* Example of parsing and storing a method *)
(* Assuming you have a function or a parsing step that adds methods to class_env *)
 ...
 this is pending  *)
  ...
  let rec initialize_class_env' cs = function
      ...
      | Class (name,super,_impl,fields,methods)::cs'  ->
        ...
        in (* Store methods using mangled names *)
        let ms = (List.map (fun (Method(n, _ret_type, pars, body)) ->
...
let new_env : string list -> env ea_result  =
  ...
  let rec lookup_method : string -> string -> class_env -> method_decl option =
    ...
    | None -> None
    | Some (_super, _fs, ms) ->
      match List.assoc_opt m_name ms with
      | Some m -> Some m
      | None -> 
        (* If not found, check the superclass with the mangled name *)
        if _super <> "" then lookup_method _super m_name c_env
        else None
...
and   
  ...
  fun e ->
  match e with
  (* Example implementation of invoke_method *)
       ...
       in (match List.assoc_opt "initialize" methods with
           ...
           | Some m -> apply_method "initialize" self args m >>= fun _ ->
             ...
             | Send(e, m_name, es) ->
              ...
              Printf.printf "Looking up %s in class %s\n" mangled_name c_name;
              (match lookup_method c_name mangled_name !g_class_env with
               | None -> Printf.printf "Method %s not found in %s\n" mangled_name c_name; error "Method not found"
  ...
  | IsEmpty(e) ->
    ...
    list_of_listVal >>= fun l ->
    return @@ BoolVal (l=[])
    | IsInstanceOf(e1, class_id) ->
      eval_expr e1 >>= obj_of_objectVal >>=
      fun obj -> return @@ BoolVal (is_subclass class_id class_id) 
  (* Debug *)
  | Debug(_e) ->
...
=======
   ...
   (* Function to check if c_name1 is a subclass of c_name2 *)
let is_subclass : string -> string -> class_env -> exp_val ea_result = 
  fun c_name1 c_name2 c_env ->
    ...
    in
    (* Check if class c_name2 exists in the environment *)
    if List.mem_assoc c_name2 c_env then
      (* Check if c_name2 is an ancestor of c_name1 *)
   ...
   and   
     ...
     | NewObject(c_name,es) ->
       ...
       | None -> return self
       | Some m -> apply_method init self args m
     >>= fun _ ->return self))
     | Send(e,m_name,es) ->
       eval_expr e >>= fun self ->
       obj_of_objectVal self >>= fun (c_name,_) ->
       eval_exprs es >>= fun args ->
       let mm_name=(name_mangle m_name es) in
       (match lookup_method c_name mm_name !g_class_env with
       | None -> error "Method not found"
       | Some m -> apply_method mm_name self args m)
     | Self ->
       eval_expr (Var "_self")
     | Super(m_name,es) ->
       eval_exprs es >>= fun args ->
