...
let rec eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  | BeginEnd(es) -> 
    (match es with
     | [] -> failwith "BeginEnd cannot be empty"
     | _ -> let rec eval_expr_sequence = function
              | [] -> return UnitVal
              | e :: es -> eval_expr e >>= fun _ -> eval_expr_sequence es
            in eval_expr_sequence es)
  | NewRef(e) ->
    eval_expr e >>= fun ev ->
  ...
  | Hd(e1)->
    eval_expr e1 >>=
    list_of_listVal >>= fun l1 ->
    if l1=[] then error "Hd: Empty list"
    else return @@ List.hd l1
  | Tl(e1)->
    eval_expr e1 >>=
    list_of_listVal >>= fun l1 ->
    if l1=[] then error "Tl: Empty list"
  ...
  | IsEmpty(e1)     ->
    eval_expr e1 >>= fun v1 ->
    if is_listVal v1 then list_of_listVal v1 >>= fun l1 -> return @@ BoolVal (l1 = [])
    else if is_treeVal v1 then tree_of_treeVal v1 >>= fun t1 -> return @@ BoolVal (t1 = Empty)
    else error "Expected a tree or a list!"
  | EmptyList(_t)    -> return @@ ListVal []
  | EmptyTree(_t) -> return @@ TreeVal Empty
  | Node(de,le,re) ->
    eval_expr de >>= fun data ->
    eval_expr le >>=
